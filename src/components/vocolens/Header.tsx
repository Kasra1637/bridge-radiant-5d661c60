...
interface HeaderProps {
  currentPage?: string;
  onNavigate?: (page: 'home' | 'resources' | 'science-of-reflection' | 'emotional-awareness-patterns') => void;
}

const resourcesDropdown = [
  {
    id: 'science-of-reflection' as const,
    icon: Brain,
    label: 'Science of Reflection',
  },
  {
    id: 'emotional-awareness-patterns' as const,
    icon: Radar,
    ...
    <>
          ...
          >
            ...
            <nav className="flex items-center gap-0.5">
              ...
              >
                ...
                >
                  <div className="p-2">
                    {resourcesDropdown.map(({ id, icon: Icon, label }) => (
                      <button
                        key={id}
                        onClick={() => {
                          setResourcesOpen(false);
      ...
      >
        ...
        <div className="absolute top-0 left-0 right-0 flex justify-center pt-[4.5rem] px-3">
          <nav
            className={`w-full bg-white rounded-[20px] overflow-hidden transition-all duration-300 ${
              isMenuOpen ? 'translate-y-0 opacity-100' : '-translate-y-4 opacity-0'
            }`}
            style={{
              boxShadow: '0 8px 40px rgba(0,0,0,0.12), 0 1px 4px rgba(0,0,0,0.05)',
            }}
          >
            <div className="px-4 py-4 space-y-1">
              ...
              <button
                onClick={() => { setIsMenuOpen(false); onNavigate?.('resources'); }}
                className="w-full text-left px-4 py-3 text-sm font-medium text-gray-600 hover:text-gray-900 hover:bg-gray-50 rounded-xl transition-colors"
              >
                Resources
              </button>
              ...
              {resourcesDropdown.map(({ id, icon: Icon, label }) => (
                <button
                  key={id}
                  onClick={() => { setIsMenuOpen(false); onNavigate?.(id); }}
